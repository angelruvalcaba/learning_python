...
parser.add_argument('--size', required=False, type=int, default=4500000,
	metavar='<str>', help='size of genome [%(default)i]')
parser.add_argument('--orfmin', required=False, type=int, default=100,
	metavar='<int>', help='min orf size [%(default)i]')
...
#get same random numbers every time you run the program. good for debugging
if arg.seed: random.seed(1)
if arg.info: print(arg.size, arg.orfmin, arg.gc)
#generate random genome of specified size and GC comp.
	...
	if seq[i: i+3] == 'ATG':
		...
		for j in range(i, len(seq)-2, 3):
			...
			if codon == 'TAA' or codon == 'TGA' or codon == 'TAG':
				stop = j
				break
	if stop != None: lengths.append((stop - start)/3) #number of amino acids, Not codons
count = 0 
for n in lengths:
	if n > arg.orfmin:
		count +=1
